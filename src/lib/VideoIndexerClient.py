...
class VideoIndexerClient:
    ...
    def __init__(self, config:Config) -> None:
        ...
        self.account = None
        self.region = config.video_indexer_region
        self.accountid = config.video_indexer_account_id

    def authenticate_async(self) -> None:
        # Get access tokens
        self.arm_access_token = get_arm_access_token(self.config)
        self.vi_access_token = get_account_access_token_async(self.config, self.arm_access_token)

        self.account = {}
        self.account["location"] = self.region
        self.account["properties"] = {}
        self.account["properties"]["accountId"] = self.accountid
    ...
    def get_account_async(self) -> None:
        ...
        url = f'{self.config.consts.azure_resource_manager}/subscriptions/{self.config.subscription_id}/resourcegroups/' + \
              f'{self.config.consts.resource_group}/providers/Microsoft.VideoIndexer/accounts/{self.config.consts.account_name}' + \
              f'?api-version={self.config.consts.api_version}'
    ...
    # video url must be SAS url to blob file
    def upload_url_async(self, internalVideoId:str,  video_name:str, video_url:str, excluded_ai:Optional[list[str]]=None,
        ...
        indexingPreset = "AudioOnly" # "BasicAudio" #"BasicAudio" #"Standard" #"Default"
        ...
        # See https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Upload-Video
        url = f'{self.config.consts.api_endpoint}/{self.account["location"]}/Accounts/{self.account["properties"]["accountId"]}/Videos'
    
        params = {
            'accessToken': self.vi_access_token,
            'name': internalVideoId,
            'privacy': privacy,
            'videoUrl': video_url,
    ...
    def wait_for_index_async(self, video_id:str, language:str='English', timeout_sec:Optional[int]=None) -> None:
        ...
        url = f'{self.config.consts.api_endpoint}/{self.account["location"]}/Accounts/{self.account["properties"]["accountId"]}/' + \
...
#https://api.videoindexer.ai/{location}/Accounts/{accountId}/Videos/{videoId}/Index[?language][&reTranslate][&includeStreamingUrls][&includedInsights][&excludedInsights][&includeSummarizedInsights][&accessToken]
    ...
    def get_insights_widgets_url_async(self, video_id:str, widget_type:str, allow_edit:bool=False) -> None:
        ...
        video_scope_access_token = get_account_access_token_async(self.config.const, self.arm_access_token,
